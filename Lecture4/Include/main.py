...
# CORS 미들웨어를 추가하여 모든 origin에서의 요청을 허용합니다.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/professor_info/{Professor}")
async def professor_info(Professor: str):
    data = session.query(models.LectureInfo).filter(models.LectureInfo.교수명 == Professor).all()
...
@app.get("/professor_repu/{Professor}")
async def professor_repu(Professor: str):
    data = session.query(models.LectureInfo).filter(models.LectureInfo.교수명 == Professor).all()
    ids = [info.강의아이디 for info in data]
    professor_data = []
    for id in ids:
